    ...
    */
     ...
     */
     if($('.typeahead')[0]) {
          ...
          var statesArray = ['Fonctionnement', 'Equipement',
          ];
     ...
     if($('.typeahead1')[0]) {
         ...
         var statesArray = ['Impots et taxes', 'Frais personnel','TRAVAUX FOUN.SCES.EXTERIEURS', 'TRANSPORT ET DEPLACEMENT'
                            ,'Terrains ', 'OEUVR D ART ET PIECESDE COLLECTION','CONSTRUCTION','MATERIEL ET OUTILLAGE','MATERIELS DE TRANSPORT'];
       var states = new Bloodhound({
           datumTokenizer: Bloodhound.tokenizers.whitespace,
           queryTokenizer: Bloodhound.tokenizers.whitespace,
           local: statesArray
...
{
	if(bool==1){$('.cardGlobale').css('display','');}
}
...
$('button.AjouterDot').on('click',function(){
	//$('.card.cardGlobale').css('display','');
	bool=1;
	afficherTable();
...
function afficherSupprMessage(compteRow){
    swal({
  title: 'Ete Vous Sure ?',
  text: "Voulez vous vraiment supprimer cette dotation!",
  type: 'warning',
  showCancelButton: true,
